...
function master_problem(params::GeneralModelParameters, sol::CCGSolutionInfo, iter::Int64, timelimit, gap, big_M)
    ...
    # Note: bounds are specific to mulitknapsack problem
    ξbases_proj = Dict(i => @variable(masterproblem, [1:params.num_ξ], lower_bound = 0.0, upper_bound = 2.0) for i in eachindex(sol.bases_constraints))
