...
function master_problem_projection(params::GeneralModelParameters, sol::CCGSolutionInfo, iter::Int64, timelimit, gap, big_M)
    ...
    ξbases_proj = Dict(i => @variable(masterproblem, [j=1:params.num_ξ], lower_bound = 0.0, upper_bound = params.upper_ξ[j]) for i in eachindex(sol.bases_constraints))
